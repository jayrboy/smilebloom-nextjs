...
const TeethPage = () => {
  ...
  useEffect(() => {
    ...
    if (childIdFromUrl === selectedChildId) return;
        ...
        {status === 'authenticated' && (
          <div className="mt-6 grid gap-6">
            ...
            <section className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-black/5">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {teethByTab
                  .filter((t) => {
                    ...
                    return last && (last.type === 'ERUPTED' || last.type === 'SHED' || last.type === 'EXTRACTED');
                  })
                  .map((t) => {
                    const last = lastEventByTooth.get(t.code);
                    const statusText =
              ...
              {loadingTeeth && (
                <div className="mt-4 text-sm text-slate-500">กำลังโหลดข้อมูลลำดับฟัน...</div>
              )}
            </section>
            ...
            {pageBusy && (
              <div className="text-center text-sm text-slate-500">กำลังโหลดข้อมูล...</div>
            )}
          </div>
        )}
      </div>
